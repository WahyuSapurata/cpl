...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'CPL');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/operator/add-cpl', 'Tambah',
                    'CPL',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/operator/update-cpl/' + uuid,
                    'Update',
                    'CPL', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/operator/show-cpl/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'CPL', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/operator/delete-cpl/' + $(this).attr('data-uuid');
                ...
                @if (auth()->user()->role != 'kajur' && auth()->user()->role != 'dosen' && auth()->user()->role != 'lpm ')
                    columnDefs: [{
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="CPL" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
