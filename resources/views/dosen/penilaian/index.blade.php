    ...
    @section('button')
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <!--begin::Title-->
                <button class="btn btn-primary btn-sm disabled-link" data-kt-drawer-show="true"
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            ...
            control.overlay_form('Tambah', 'Penilaian');
        });
        ...
        $(document).ready(function() {
            $(document).on('click', '#button-cari', function(e) {
                ...
                $('#tThead').empty();
                $('#button-side-form').removeClass('disabled-link');

                // Kosongkan elemen teknik-penilaian
                $('#teknik-penilaian').empty();

                const matkul = $('#from_select_matkul').val();
                const cpmk = $('#from_select_cpmk').val();
                ...
                $.ajax({
                    url: `/dosen/get-penilaian`,
                    method: 'GET',
                    data: data,
                    success: function(res) {
                        let teknikPenilaian = res.data.teknik_penilaian.map(item => item
                            .teknik_penilaian);
                        let teknikPenilaianHead = teknikPenilaian.map(teknik =>
                            `<th>${teknik}</th>`).join('');
                        // $('#tThead').html(
                        //     `<th>No</th><th>Nama Mahasiswa</th>${teknikPenilaianHead}`);

                        res.data.teknik_penilaian.forEach(item => {
                            $('#teknik-penilaian').append(
                                `
                        ...
                        let label_head = ['nama_mahasiswa', ...teknikPenilaian, 'uuid'];
                        ...
                        initDatatable(combinedData, teknikPenilaianHead, label_head);
                    },
            ...
            data.forEach(item => {
                ...
                if (!combined[nama_mahasiswa]) {
                    combined[nama_mahasiswa] = {
                        nama_mahasiswa,
                        uuid: [], // Inisialisasi array untuk UUID
                    };
                }

                if (item.uuid) combined[nama_mahasiswa].uuid.push(item.uuid);

                Object.keys(item).forEach(key => {
                    if (key !== 'nama_mahasiswa' && key !== 'uuid') {
        ...
        const initDatatable = async (data, teknikPenilaianHead, label_head) => {
            // Buat elemen No dengan atribut class yang sesuai
            let tr = `<tr><th>No</th><th>Nama Mahasiswa</th>${teknikPenilaianHead}<th>Aksi</th></tr>`;
            ...
            $('#kt_table_data').DataTable({
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        <a href="javascript:;" type="button" data-uuid="${data}" data-label="Penilaian" class="btn btn-danger button-delete btn-icon btn-sm">
                            <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            // Tambahkan tahun_ajaran ke form data
            let formData = new FormData(this);
            formData.append('tahun_ajaran', tahun_ajaran);

            if (type == 'add') {
                control.submitForm('/dosen/add-penilaian', 'Tambah', 'Peniaian', 'POST', formData);
                $('#button-cari').click();
            } else {
                control.submitForm('/dosen/update-penilaian', 'Update', 'Peniaian', 'POST', formData);
                $('#button-cari').click();
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            $(".title_side_form").html(`Update Penilaian`);
            const uuidSubCpmks = [];

            $('input[name="uuid_sub_cpmks[]"]').each(function() {
                uuidSubCpmks.push($(this).val());
            });

            // Gabungkan uuidMahasiswa dengan uuidSubCpmks ke dalam objek
            // Ambil UUID dari atribut data-uuid, dan pastikan hanya satu UUID yang diambil
            let uuids = $(this).attr('data-uuid').split(',');
            let uuidNilai = uuids[0]; // Ambil UUID pertama

            const combinedUuid = {
                uuid: uuidNilai,
                uuid_sub_cpmks: uuidSubCpmks
            };

            $(".form-data").attr("data-type", "update");

            // Kirim objek combinedUuid ke backend menggunakan AJAX GET request
            $.ajax({
                url: '/dosen/show-penilaian',
                method: 'GET',
                ...
                success: function(res) {
                    if (res.success === true) {
                        $.each(res.data, function(x, y) {
                            if (Array.isArray(y)) {
                                let teknikPenilaianInputs = $("input[name='nilai[]']");
                                y.forEach((item, index) => {
                                    teknikPenilaianInputs.eq(index).val(item);
                                });
                            } else {
                                $("input[name='" + x + "']").val(y);
                                $("select[name='" + x + "']").val(y);
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/dosen/delete-penilaian/' + $(this).attr('data-uuid');
